...
/**
 * SRS・亥ｿ伜唆譖ｲ邱夲ｼ芽ｨｭ螳壹お繝・ぅ繧ｿ
 * 
 * 蠕ｩ鄙帝俣髫斐・繝励Μ繧ｻ繝・ヨ繧堤ｮ｡逅・＠縺ｾ縺吶・
 */
export function SRSEditor() {
    ...
    const confirmAdding = async () => {
        ...
        try {
            ...
            alert("Failed to add SRS profile.");
        }
    };
    ...
    const executeDelete = async (id: string) => {
        try {
            ...
            alert("Failed to delete SRS profile.");
        }
    };
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-gray-500 dark:text-gray-400">SRS Settings</h3>
                {!isAdding && (
                    <button
                        onClick={startAdding}
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800 rounded-lg p-3 mb-3">
                        <div className="space-y-2">
                            <input
                                ...
                                placeholder="Name (e.g. Short Term)"
                                autoFocus
                            />
                            <input
                                type="text"
                                ...
                                className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded px-2 py-1 text-sm focus:outline-none focus:border-blue-500"
                                placeholder="Intervals (e.g. 1, 3, 7)"
                                onKeyDown={(e) => {
                            ...
                            <div className="flex justify-end space-x-2 mt-2">
                                <button onClick={cancelAdding} className="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">繧ｭ繝｣繝ｳ繧ｻ繝ｫ</button>
                                <button onClick={confirmAdding} className="text-xs bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">菫晏ｭ・/button>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div key={profile.id} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center space-x-2">
                                ...
                                {profile.isDefault && (
                                    <span className="text-[10px] bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-1.5 py-0.5 rounded">繝・ヵ繧ｩ繝ｫ繝・/span>
                                )}
                                    </div>
                            {!profile.isDefault && (
                                    <button
                                    ...
                                    >
                                        {deleteConfirmId === profile.id ? "Delete" : <Trash2 size={16} />}
                                    </button>
                                )}
                            </div>
