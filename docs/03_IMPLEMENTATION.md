...
- **ルーティーン機能 (Mobile)**:
    - **作成**: Todo作成時に曜日を指定すると、向こう30日分のタスクを一括生成。
    - **編集**: Todo詳細画面からルーティーン設定（曜日）を変更可能に。
        - 変更時は既存のグループタスク（同じ`srsGroupId`を持つもの）を全て削除し、新しい設定で再生成する仕様を採用。
    - **レイアウト**: Todo詳細画面のルーティーンボタンを、作成画面に合わせてアイコンのみのコンパクトな表示に調整。
