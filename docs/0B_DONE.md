...
### フェーズ 5: モバイル版の仕上げとストア公開 (完了済み項目)
- [x] **シェア機能の実装 (Mobile)**: Web版同様の画像生成とSNS共有機能。
- [x] **ローカル通知機能 (Mobile)**: タイマー完了通知とデイリーリマインダーの実装。
- [x] **ストア公開準備**: スクリーンショット作成、説明文、プライバシーポリシー設定。
- [x] **iOSリリース準備**: クラウド Mac を使用した iOS シミュレータ撮影計画を策定。
- [x] **iOSビルド・申請**: Apple Developer Program 登録後、EASで実行。
- [x] **iOS App Store Connect提出**: EAS SubmitによりTestFlightおよび審査提出用のバイナリをアップロード完了 (2026-03-05)。
- [x] **Sharedパッケージ復旧確認**: 破損疑いの検証とテスト通過確認。
- [x] **AdMob初期化不具合の修正**: EAS Secrets (env) を設定し、TestFlightで広告が表示されるよう修正済。
- [x] **EAS Build環境変数の設定漏れ修正**: `.env`がgitignoreされておりSupabase認証が失敗していた問題を解消 (2026-03-07)。
- [x] **パスワードリセット完結画面の実装**: Web版に `PASSWORD_RECOVERY` イベント検出 → 新パスワード設定UIを追加。モバイル版はWebへリダイレクトする方式で対応。
- [x] **AIエージェント向け共通ルールの整備**: Claude Code・Gemini等で相互利用可能なプロジェクトルールを設定ファイルに集約 (2026-03-07)。
- [x] **Android クローズドテスト**: プロダクションビルドの作成・Google Play Consoleへのアップロード・パスワードリセット画面実装・広告バナー修正。

### フェーズ 6: モバイルリリース後の展開 (完了済み項目)
- [x] **Androidエミュレータ環境の構築**: 実機レスでの開発・テスト環境を整備。
- [x] **SNS共有による期間限定広告OFF機能**: 共有UIの実装と連動。
- [x] **本番ビルド (EAS Build)**: Android/iOS 設定完了。
- [x] **ストア申請**: Google Play Console / App Store Connect へのアップロード。

### 完了済みリファクタリング / 技術的負債
| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| 1 | **`pomarc` 残存の修正（6箇所）** | ✅ 完了 | マイグレーションユーティリティ作成 (2026-02-28) |
| 2 | **不要ファイルの除外** | ✅ 完了 | `.gitignore` + `git rm --cached` (2026-02-28) |
| 3 | **巨大コンポーネントの分割** | ✅ 完了 | 4コンポーネントからカスタムフック抽出、約44%行数削減 (2026-02-28) |
| 4 | **MainLayout デッドコード削除** | ✅ 完了 | 共通ハンドラ9個を `useTodoHandlers.ts` に抽出 (2026-02-28) |
| 5 | **同期ロジックのテスト追加** | ✅ 整備済み | 93件全pass (2026-02-28確認) |
| 6 | **Web/Mobile共通ロジック抽出** | ✅ 完了 | コア同期ロジックの重複を排除 (2026-02-28) |
| 7 | **`useTimer.test.ts.skip` の対応** | ✅ 完了 | timerCore.ts に純粋関数5個抽出、24テスト全pass (2026-02-28) |
| 8 | **高速化** | ⏭️ 対応不要 | 体感速度問題なし、必要時に再検討 |

### その他の完了済み項目
- [x] **共通ロジックの統合**: TodoDetailModalのテキスト解析ロジックをpackages/sharedへ移動。
- [x] **テストコードの拡充**: テストカバレッジ 84→120→188件。(2026-02-28)
...
- [x] **Google Analytics 4 導入 (2026-03-05)**: 環境変数ベースのGA4計測コード追加。
- [x] **PWA対応 (2026-03-05)**: manifest.json作成、layout.tsxへのmanifestリンク追加。
- [x] **ランディングページ強化 (2026-03-05)**: Stats/How It Works/Bottom CTAセクション追加、i18n対応。
...
### [Timer UI Redesign] - 2026-03-08
#### 修正 (Mobile)
- **タイマー画面のUI改善**:
    - 開始/停止ボタンをタイマー円内に配置し、時間表示の下に重ねる方式に変更。独立したControlsセクションを削除。
    - ボタンカラーを薄い半透明（`rgba`）に変更し、視覚的に軽くなるデザインに。
    - タイマー円のサイズを画面幅の88%→75%に縮小し、下部の「記録のみ保存」「タスク完了」ボタンが見切れる問題を解消。
    - 時間表示のフォントサイズを80→64に調整。

#### 修正 (Web)
- **タイマー画面のUI改善 (Mobile版と同様)**:
    - 開始/停止ボタンをタイマー円内に配置（薄い半透明カラー: `bg-blue-600/15` 等）。独立Controlsセクション削除。
    - **タスク完了ボタンを新規追加**: モバイル版と同様に、タイマー画面からタスクを完了にできる機能を追加。
    - タスク完了時は未保存セッションを自動保存し、todoの`completed`を`true`に更新してホーム画面に戻る。
    - タイマー終了時のセッション自動保存機能を追加（ポモドーロ集中終了時・カウントダウン終了時）。

#### 機能追加 (Web + Mobile)
- **ポモドーロ休憩時間のオート進行（トグル機能付き）**:
    - 集中時間終了後に自動的に休憩タイマーを開始。休憩終了後も自動的に次の集中タイマーを開始。
    - ...（MoreVertical）ボタンにドロップダウンメニューを新設。オート進行ON/OFFトグル＋記録ログ表示。
    - デフォルトはオート進行ON。
- **Web版にタイマーログ（記録ログ）機能を追加**:
    - Mobile版と同様の記録ログモーダルを実装。Dexie DBからセッション情報を直接取得。
    - 各セッションの日時・時間・モードを一覧表示。合計時間も表示。
- **タイマー画面のダークモード対応 (Web + Mobile)**:
...
#### ドキュメント改訂 (Docs)
- **PRO_PLAN.md の大幅改訂**:
    - **多端末同期の無料化**: Pro特典から除外し、ログインユーザー全員が利用可能な無料機能に変更。
    - **詳細分析・エクスポート（#4, #5）を保留**: 確定機能から保留に移動。
    - **機能#6〜#10を保留**: 検討中機能をすべて保留に変更。
    - **クロスプラットフォーム課金の重複防止セクション（5.4）を新設**: iOSで課金済みのユーザーがWeb版で重複課金できない仕組みを設計。
    - **ペイウォールUIの簡素化**: 多端末同期・詳細分析の訴求を削除。
    - **未決定事項の整理**: 解決済み項目を明記。
- **GAMIFICATION_PLAN.md を新規作成**:
    - ゲーミフィケーション機能（ストリーク、バッジ、レベル/XP、学習カレンダー）の計画書を別ドキュメントとして独立化。
    - 無料/Pro の線引き案を記載。
